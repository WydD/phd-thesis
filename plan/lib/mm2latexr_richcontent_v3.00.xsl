	...
	<xsl:for-each select="$i">
		<xsl:choose>
			...
			<xsl:when test="current()/richcontent/html/body/p/@text">
				<xsl:text>	\item </xsl:text><xsl:call-template name="richtext"/>
			</xsl:when>
			<xsl:otherwise>
			</xsl:otherwise>
		</xsl:choose>
...
<xsl:template name="figures">
	<xsl:text>
		\includegraphics[width=1.0\textwidth]{</xsl:text><xsl:value-of select="current()/node/richcontent/html/body/img/@src"/><xsl:text>}
	</xsl:text>
</xsl:template>
<!-- template to parse and insert figures with manually edited html. (inside <p>)-->
<xsl:template name="figuresp">
	<xsl:text>
		\includegraphics[width=1.0\textwidth]{</xsl:text><xsl:value-of select="current()/node/richcontent/html/body/p/img/@src"/><xsl:text>}
	</xsl:text>
</xsl:template>
