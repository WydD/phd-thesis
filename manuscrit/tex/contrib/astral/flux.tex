...
\begin{defi}[\textit{Streamers} sensibles]
    Soit $R$ une relation, alors le \textit{streamer} d'insertion $\IS(R)=S$ est défini par la propriété suivante : $\forall (t,i)\in\TN$
        $$s\in R(t,i) \wedge s\not\in R((t,i)^-) \equ s'=\Psi_{(t,i)}(s)\in S \wedge \BS(s') = (t,i)$$
\end{defi}
...
\end{itemize}
L'expression exacte de l'application $\Phi_b^S$ n'a pas beaucoup d'importance car ses propriétés suffisent à rendre son utilisation libre de toute ambiguïté. Il est important de
\subsection{Domaine}
\subsection{Spread}