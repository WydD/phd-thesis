...
D'un point de vue modélisation, le modèle en étoile ou en flocon permettent de structurer correctement un ensemble de données multidimensionnel~\cite{Levene:snowflake}. Le principe étant qu'il existe une relation des \textit{faits}. Celle-ci contiendra l'ensemble des données brutes. Dans notre cas, ce sera le prix. Cette relation est ensuite liée à plusieurs autres relations qui décrivent les différentes dimensions. Dans la structure en étoile, chaque dimension correspond à une relation. Dans le cadre de la structure en flocon, chaque dimension est un modèle relationnel normé (en \textit{3NF}). 
...
\begin{table}[!ht]
\criteretabDonnee
    {Modèle relationnel, 3NF, Modèle en étoile.}
