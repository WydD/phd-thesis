...
\def\newthmtype#1#2#3{
	...
	\newenvironment{#1}[1][]
		{\vskip .4cm \refstepcounter{#1ct}\begin{thmbox}[#3]{\ifx#3S\it\else\fi #2 \chapterarabic{#1ct}\phantom{y} ( {\bf ##1} )} 
    \let\BODY\undefined
    \setlength{\thmsize}{\linewidth}
    \addtolength{\thmsize}{-5em}
    \addtolength{\thmsize}{-0.3em}
    \newsavebox{\BODY} % Box storage
    \begin{lrbox}{\BODY} % Start capturing the input 
\hskip 1em \begin{minipage}{\thmsize}
\vskip 1em
	}
		{\par \quad
\end{minipage} \end{lrbox}\colorbox{good}{\usebox{\BODY}\hskip 1em} 
