...
\def\newthmtype#1#2#3{
        ...
        \newfloat{#1float}{ht}{#1toc}[section]\floatname{#1float}{#2}
	\expandafter\def\csname the#1float\endcsname{\sectionarabic{#1float}}
...
\newenvironment{absexample}[1][\examplename]{\par\vspace{1em}%
                    \setlength{\thmsize}{\linewidth}%
                    \addtolength{\thmsize}{-2\stdindent}%
   \noindent\textbf{#1\, :}\phantom{p} \par \noindent\hspace{\stdindent}\begin{minipage}[b]{\thmsize}\setlength{\parindent}{\stdindent}
   
  }{\end{minipage} \hfill $\lhd$ \vspace{1em}}
%  \let\demo=\comment
