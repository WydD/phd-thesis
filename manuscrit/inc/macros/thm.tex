...
\definecolor{deficolor}{RGB}{234,255,234}
\definecolor{propcolor}{RGB}{234,244,255}
\definecolor{thmcolor}{RGB}{255,234,234}
...
% #3 = style (S - M)
\def\newthmtype#1#2#3{
        \DeclareCaptionFormat{#1label}{{\ifx#3S\it\else\bf\fi ##1}\hfill \phantom{p}  {\bf ##3} \phantom{M}}
        \newfloat{#1float}{ht}{#1hf}[chapter]\floatname{#1float}{#2}
        \captionsetup[#1float]{position=above,skip=0pt,format=#1label}
	\expandafter\def\csname the#1float\endcsname{\chapterarabic{#1float}}
	\newenvironment{#1}[1][]
		{ \begin{#1float}[ht]\caption{##1}  \begin{center} 
                    \let\BODY\undefined
                    \setlength{\thmsize}{\linewidth}
                    \addtolength{\thmsize}{-5em}
                    \newsavebox{\BODY} % Box storage
                    \begin{lrbox}{\BODY} % Start capturing the input 
                        \hskip 1em \noindent\begin{minipage}{\thmsize}
                        \vskip 1em
                }
		{
                        \par \quad \end{minipage}
                    \end{lrbox}
                    \fcolorbox{black}{#1color}{\usebox{\BODY}\hskip 1em} \end{center}
                \end{#1float}}
	\newenvironment{#1*}[1][]
